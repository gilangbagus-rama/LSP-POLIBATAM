...
<head>
  ...
  <title>SI-LSP | Ganti E-Mail Verifikasi</title>
  <!-- Favicon -->
    ...
    <div class="login-box-body">
      <form action="" method="post">
        ...
        <div class="form-group">
          <button type="submit" class="btn btn-primary btn-block btn-flat" name="ganti_email" value="ganti_email"
          data-loading-text="<i class='fa fa-spinner fa-spin'></i> Loading">Ganti E-Mail Verifikasi</button>
        </div>
          ...
          if($_POST['ganti_email']) {
            ...
            if ( mysqli_num_rows($cekEMAIL) == 1 ) {
              ...
              echo 
                ...
                role="alert">E-Mail telah digunakan.</div>';
            ...
            // Buat Token Reset Password
              ...
              $nama = $conn->query("SELECT `akun`.`email` , `akun`.`nama` FROM `akun`,`asesi` WHERE `akun`.`email` = `asesi`.`email` AND `asesi`.`id_asesi` = '$id_asesi' ");
              $dat = mysqli_fetch_assoc($nama); $nama = $dat['nama']; $email_lama = $dat['email'];

              $email_verif = base64_encode($email) ;

              $link_batal = 'http://localhost/lsp/index.php?page=batal-daftar&&email=' .$email_verif. '&&token=' .$token;

              $link = 'http://localhost/lsp/index.php?page=auth&&email=' .$email_verif. '&&token=' .$token ;


              $header_email = '<b>Yth. Bapak/Ibu ' .$nama. '</b> <br>';


              $body_email = '<br>Berikut ini adalah link untuk melakukan verifikasi ulang akun Anda <br>' .$link;

              $body_email = $body_email. '<br>Jika Anda merasa tidak melakukan permintaan ini, silahkan klik link berikut <br>';
            ...
            if(!$mail->send()) {
              ...
              $sql_input_token = $conn->query("UPDATE `akun` SET `email` = '$email' ,`token_verif` = '$token' WHERE `email` = '$email_lama' ");
